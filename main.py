...
def gerar_sugestao(stats_home, stats_away):
    try:
        ...
        btts_media = btts_home + btts_away
        ...
        # Under 3.5
        if gm_home <= 1.0 and gm_away <= 1.0 and clean_home + clean_away >= 6 and shots_on_home + shots_on_away < 7 and btts_media < 120:
            alta_conf.append("🧤 Under 3.5 gols (alta)")
        elif gm_home <= 1.3 and gm_away <= 1.3 and clean_home + clean_away >= 4 and shots_on_home + shots_on_away < 8 and btts_media < 130:
